...
argParser.add_argument("-usr", "--user", required=True, type=str, metavar="snmp-user", dest="snmpUsername",
	help="Username for connecting to SNMP service (SNMPv3)")
argParser.add_argument("-akey", "--authkey", required=True, type=str, metavar="auth-pass", dest="snmpAuthKey",
	help="Authentication passphrase for connecting to SNMP service (SNMPv3)")
argParser.add_argument("-pkey", "--privkey", required=True, type=str, metavar="privacy-pass", dest="snmpPrivKey",
	help="Privacy passphrase for connecting to SNMP service (SNMPv3)")
...
argParser.add_argument("-ilim", "--iterlim", required=False, type=int, default=256, choices=range(1, 65536), metavar="(1 .. 65535)", dest="snmpIterMaxCount",
	help="SNMP count values limit for iterable objects (default is 256)")
argParser.add_argument("-rcnt", "--retriescnt", required=False, type=int, default=0, choices=range(0, 10), metavar="(0 .. 9)", dest="snmpRetriesCount",
	help="SNMP request retries count (default is 0)")
argParser.add_argument("-tout", "--timeout", required=False, type=float, default=2.0, metavar="(0 .. 20.0)", dest="snmpTimeout",
	help="SNMP request timeout in seconds (default is 2.0)")
argParser.add_argument("-iping", "--ignoreping", action="store_true", dest="ignorePingFlag",
	help="When set, ignores results of an ICMP PING scan (checks every host using SNMP requests)")
argParser.add_argument("-out", "--report", required=False, type=str, metavar=".\Report.csv", dest="outFilePath",
	help="Path to report output file, including extension (autogenerated in script directory by default)")
argParser.add_argument("-empty", "--emptyvalue", required=False, type=str, default="N/A", metavar="\"N/A\"", dest="reportEmptyValue",
	help="Empty value representation for the report (default is \"N/A\")")
argParser.add_argument("-csvdel", "--csvdelimiter", required=False, type=str, default=";", metavar="\";\"", dest="csvReportDelimeter",
	help="Delimiter symbol for the CSV report (default is \";\")")
...
def snmpAudit(snmpHost, pingStatus, snmpUsername, snmpAuthKey, snmpPrivKey, dataDict, valuesDelimeter=";", snmpAuthProtocol=usmHMACSHAAuthProtocol, snmpPrivProtocol=usmAesCfb128Protocol, snmpPort=161, snmpIterMaxCount=256, snmpRetriesCount=0, snmpTimeout=2.0):
	# Function variables
